...
function EditOrderContent() {
  ...
  const exportToExcel = () => {
    ...
    data.push(['物件名', order.basicInfo.propertyName || '']);
    data.push(['見積日', order.basicInfo.estimateDate ? order.basicInfo.estimateDate.toISOString().split('T')[0] : '']);
    data.push(['受注日', order.basicInfo.orderDate ? order.basicInfo.orderDate.toISOString().split('T')[0] : '']);
    data.push(['支払日', order.basicInfo.paymentDate ? order.basicInfo.paymentDate.toISOString().split('T')[0] : '']);
    data.push(['納品日', order.basicInfo.deliveryDate ? order.basicInfo.deliveryDate.toISOString().split('T')[0] : '']);
    data.push(['担当者', order.basicInfo.assignedStaff || '']);
    data.push(['受注見積金額', order.basicInfo.estimateAmount || 0]);
    data.push(['原価金額', order.basicInfo.costAmount || 0]);
    
    // 現場情報
    data.push(['=== 現場情報 ===']);
    data.push(['現場担当者', order.siteInfo.siteManager || '']);
    data.push(['取引先担当者', order.siteInfo.clientManager || '']);
    data.push(['物件住所', order.siteInfo.propertyAddress || '']);
    data.push(['オートロック№', order.siteInfo.autolockNumber || '']);
    
    // 点検情報
    data.push(['=== 点検情報 ===']);
    data.push(['点検周期', order.inspectionInfo.inspectionCycle || '']);
    data.push(['作成日', order.inspectionInfo.creationDate ? order.inspectionInfo.creationDate.toISOString().split('T')[0] : '']);
    data.push(['部数', order.inspectionInfo.copies || 1]);
    data.push(['確認', order.inspectionInfo.confirmation || '']);
    data.push(['提出日', order.inspectionInfo.submissionDate ? order.inspectionInfo.submissionDate.toISOString().split('T')[0] : '']);
    data.push(['作成者', order.inspectionInfo.creator || '']);
    data.push(['備考', order.inspectionInfo.remarks || '']);
    data.push(['点検表データ', order.inspectionInfo.inspectionData || '']);
    
    // 工事費用
    data.push(['=== 工事費用 ===']);
    data.push(['機器取付工事費（金額）', order.constructionCosts.equipmentInstallationCost.amount || 0]);
    data.push(['機器取付工事費（原価）', order.constructionCosts.equipmentInstallationCost.cost || 0]);
    data.push(['配線工事費（金額）', order.constructionCosts.wiringCost.amount || 0]);
    data.push(['配線工事費（原価）', order.constructionCosts.wiringCost.cost || 0]);
    data.push(['調整・試験費（金額）', order.constructionCosts.adjustmentTestCost.amount || 0]);
    data.push(['調整・試験費（原価）', order.constructionCosts.adjustmentTestCost.cost || 0]);
    data.push(['雑材・消耗品費（金額）', order.constructionCosts.miscellaneousSuppliesCost.amount || 0]);
    data.push(['雑材・消耗品費（原価）', order.constructionCosts.miscellaneousSuppliesCost.cost || 0]);
    data.push(['法定福利費（金額）', order.constructionCosts.legalWelfareCost.amount || 0]);
    data.push(['法定福利費（原価）', order.constructionCosts.legalWelfareCost.cost || 0]);
    data.push(['交通費（金額）', order.constructionCosts.transportationCost.amount || 0]);
    data.push(['交通費（原価）', order.constructionCosts.transportationCost.cost || 0]);
    data.push(['諸経費（金額）', order.constructionCosts.generalExpenses.amount || 0]);
    data.push(['諸経費（原価）', order.constructionCosts.generalExpenses.cost || 0]);
    data.push(['廃棄物収集保管運搬費（金額）', order.constructionCosts.wasteCollectionCost.amount || 0]);
    data.push(['廃棄物収集保管運搬費（原価）', order.constructionCosts.wasteCollectionCost.cost || 0]);
    data.push(['出精値引き', order.constructionCosts.precisionDiscount || 0]);
    
    // 明細情報
    data.push(['=== 明細情報 ===']);
    order.details.forEach((detail, index) => {
      data.push([`明細${index + 1}: NO`, detail.no || '']);
      data.push([`明細${index + 1}: 名称`, detail.name || '']);
      data.push([`明細${index + 1}: 品名コード`, detail.productCode || '']);
      data.push([`明細${index + 1}: 品名`, detail.productName || '']);
      data.push([`明細${index + 1}: 数量`, detail.quantity || 0]);
      data.push([`明細${index + 1}: 単位`, detail.unit || '']);
      data.push([`明細${index + 1}: 単価`, detail.unitPrice || 0]);
      data.push([`明細${index + 1}: 原価`, detail.cost || 0]);
      data.push([`明細${index + 1}: 税率`, detail.taxRate || 10]);
      data.push([`明細${index + 1}: 歩掛`, detail.workload || 0]);
      data.push([`明細${index + 1}: 配線`, detail.isWiring ? 'あり' : 'なし']);
      data.push([`明細${index + 1}: 発注`, detail.isOrdered ? 'はい' : 'いいえ']);
      data.push([`明細${index + 1}: 発注先コード`, detail.supplierCode || '']);
      data.push([`明細${index + 1}: 発注先名称`, detail.supplierName || '']);
    });
    ...
    const filename = `受注データ_${order.basicInfo.orderNumber || 'EDIT'}_${timestamp}.xlsx`;
    
    // ファイルダウンロード
    XLSX.writeFile(workbook, filename);
  };

  // 初期データの取得と既存受注データの読み込み
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        // 既存受注データの読み込み
        if (orderId) {
          const existingOrder = await firestoreOrderStore.getOrderById(orderId);
          if (existingOrder) {
            // 詳細データが保存されている場合は復元、なければデフォルト値を使用
            let detailOrder = null;
            if (existingOrder.detailData) {
              try {
                detailOrder = JSON.parse(existingOrder.detailData);
                // Dateオブジェクトを復元
                if (detailOrder) {
                  if (detailOrder.basicInfo) {
                    if (detailOrder.basicInfo.estimateDate) detailOrder.basicInfo.estimateDate = new Date(detailOrder.basicInfo.estimateDate);
                    if (detailOrder.basicInfo.orderDate) detailOrder.basicInfo.orderDate = new Date(detailOrder.basicInfo.orderDate);
                    if (detailOrder.basicInfo.paymentDate) detailOrder.basicInfo.paymentDate = new Date(detailOrder.basicInfo.paymentDate);
                    if (detailOrder.basicInfo.deliveryDate) detailOrder.basicInfo.deliveryDate = new Date(detailOrder.basicInfo.deliveryDate);
                  }
                  if (detailOrder.inspectionInfo) {
                    if (detailOrder.inspectionInfo.creationDate) detailOrder.inspectionInfo.creationDate = new Date(detailOrder.inspectionInfo.creationDate);
                    if (detailOrder.inspectionInfo.submissionDate) detailOrder.inspectionInfo.submissionDate = new Date(detailOrder.inspectionInfo.submissionDate);
                  }
                }
              } catch (e) {
                console.error('詳細データの解析エラー:', e);
              }
            }

            // データストアの形式から編集フォームの形式に変換
            setOrder(detailOrder || {
              basicInfo: {
                orderNumber: existingOrder.orderNo,
                projectCode: '',
                projectName: existingOrder.title,
                customerCode: '',
                customerName: existingOrder.client,
                propertyName: existingOrder.projectName,
                estimateDate: existingOrder.estimateDate,
                orderDate: existingOrder.orderDate,
                paymentDate: new Date(),
                deliveryDate: existingOrder.deliveryDate,
                assignedStaff: existingOrder.assignee,
                estimateAmount: existingOrder.amount,
                costAmount: 0
              },
              controls: {
                isOrdered: existingOrder.isOrdered,
                showDetails: true,
                isRedSlip: false,
                isInstructionOnly: false,
                instructionType: 'construction'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // データストアを更新
      const updatedOrder = await firestoreOrderStore.updateOrder(orderId, {
      ...
      if (updatedOrder) {
        console.log('受注更新完了:', updatedOrder);
        alert('受注情報を更新しました！');
        router.push('/orders');
      } else {
        throw new Error('受注の更新に失敗しました');
      }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-900">受注編集</h1>
                <p className="text-xs text-gray-500">51項目対応受注システム - {order.basicInfo.orderNumber}</p>
              </div>
            </div>
            <Link
              href="/orders"
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <input
                  type="date"
                  value={order.basicInfo.estimateDate instanceof Date ? order.basicInfo.estimateDate.toISOString().split('T')[0] : ''}
              ...
              <div>
                ...
                <input
                  type="date"
                  value={order.basicInfo.orderDate instanceof Date ? order.basicInfo.orderDate.toISOString().split('T')[0] : ''}
              ...
              <div>
                ...
                <input
                  type="date"
                  value={order.basicInfo.paymentDate instanceof Date ? order.basicInfo.paymentDate.toISOString().split('T')[0] : ''}
              ...
              <div>
                ...
                <input
                  type="date"
                  value={order.basicInfo.deliveryDate instanceof Date ? order.basicInfo.deliveryDate.toISOString().split('T')[0] : ''}
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <input
                  type="date"
                  value={order.inspectionInfo.creationDate instanceof Date ? order.inspectionInfo.creationDate.toISOString().split('T')[0] : ''}
              ...
              <div>
                ...
                <input
                  type="date"
                  value={order.inspectionInfo.submissionDate instanceof Date ? order.inspectionInfo.submissionDate.toISOString().split('T')[0] : ''}
